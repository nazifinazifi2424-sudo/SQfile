...
def track_vip_join(update):
    ...
    if update.new_chat_member.status == "member":
        ...
        if VIP_DURATION_UNIT == "minutes":
            expire_time = start_time + timedelta(minutes=VIP_DURATION_VALUE)
        else:
            expire_time = start_time + timedelta(days=VIP_DURATION_VALUE)
        ...
        cur.execute(
            ...
            WHERE user_id=%s
            """,
            (start_time, expire_time, user_id)
        )
        ...
        try:
            ...
            for user_id, expire_at in users:
                ...
                if remaining <= 0:
                    ...
                    cur.execute(
                        "UPDATE vip_members SET status='expired' WHERE user_id=%s",
                        (user_id,)
                    )
...
An cire ka daga VIP group saboda lokacin ya cika.

Domin ci gaba da morewa da duk abubuwan VIP,
...
""",
                ...
                # ================= WARNING SYSTEM =================
                    ...
                    if VIP_DURATION_UNIT == "days":
                        warning_1 = 2 * 24 * 3600   # saura kwanaki 2
                        warning_2 = 1 * 24 * 3600   # saura kwana 1
                    else:
                        warning_1 = 60   # saura minti 1
                        warning_2 = 30   # saura sakan 30

                    # ===== Gargadi na Farko =====
                    if warning_2 < remaining <= warning_1:
                        bot.send_message(
                            user_id,
                            """⏳ <b>Gargadi!</b>
...
Domin kada a cire ka daga group,
yana da kyau ka sabunta biyan kuɗinka tun yanzu.
""",
                            parse_mode="HTML",
                            reply_markup=vip_sub_button()
                        )
...
Idan baka sabunta ba,
za a cire ka daga VIP group ta atomatik.

Ka sabunta yanzu domin kada ka rasa damar shiga.
""",
                            parse_mode="HTML",
                            reply_markup=vip_sub_button()
                        )
