...
# ==================================================    
# CHECKOUT (CART) - MESSAGE HANDLER WITH STRONG DEBUGS   
# ==================================================    
@bot.message_handler(func=lambda m: m.text == "checkout")
def handle_checkout_msg(m):
    uid = m.from_user.id

    try:
        bot.send_message(ADMIN_ID, f"🔍 DEBUG: Mutum {uid} ya danna checkout. An fara aiki...")
    except:
        pass

    rows = get_cart(uid)    
    if not rows:    
        try: bot.send_message(ADMIN_ID, f"⚠️ DEBUG: Cart din {uid} babu komai (Empty rows).") # DB check
        except: pass
        bot.send_message(uid, "❌ Cart ɗinka babu komai.")    
        return    
    ...
    for item_id, title, price, file_id, group_key in rows:    
        ...
        if not file_id:    
            try: bot.send_message(ADMIN_ID, f"⚠️ DEBUG: An samu item babu file_id (item_id: {item_id})")
            except: pass
            continue    

        p = int(price or 0)    
        if p <= 0:    
            try: bot.send_message(ADMIN_ID, f"⚠️ DEBUG: An samu item farashi 0 ko kasa da haka (item_id: {item_id})")
            except: pass
            continue    
    ...
    if not groups:    
        try: bot.send_message(ADMIN_ID, f"⚠️ DEBUG: Idanun groups sun fito empty bayan tacewa (Uid: {uid}).")
        except: pass
        bot.send_message(uid, "❌ Babu item mai delivery a cart.")    
        return    
    ...
    if total <= 0:    
        try: bot.send_message(ADMIN_ID, f"⚠️ DEBUG: Total price ya zama 0 ko kasa da haka: {total}")
        except: pass
        bot.send_message(uid, "❌ Farashi bai dace ba.")    
        return    
    ...
    try:    
        ...
        conn.commit()    
        try: bot.send_message(ADMIN_ID, f"⚡ DEBUG: DB commit anyi nasara! Order_ID: {order_id}")
        except: pass

    except Exception as db_err:    
        if conn:    
            conn.rollback()    
        try: bot.send_message(ADMIN_ID, f"❌ DEBUG: DB Error (Checkout failed) na auku: {str(db_err)}")
        except: pass
        bot.send_message(uid, "❌ Checkout failed.")    
        return    
    ...
    clear_cart(uid)    
    try: bot.send_message(ADMIN_ID, f"🧹 DEBUG: An goge cart na user {uid} lafiya. An nufi wajen payment link...")
    except: pass

    # Anan ne kace yana tsayawa - Mu duba ko Payment Link ne baya fitowa
    try:
        pay_url = create_paystack_payment(uid, order_id, total, "Cart Order")    
    except Exception as pay_err:
        try: bot.send_message(ADMIN_ID, f"❌ DEBUG: Error lokacin kiran create_paystack_payment: {str(pay_err)}")
        except: pass
        pay_url = None

    if not pay_url:    
        try: bot.send_message(ADMIN_ID, f"❌ DEBUG: Payment function ta dawo da Empty/None URL (Flutterwave ko Paystack ba su bada link ba).")
        except: pass
        return    
...
"""
        try: bot.send_message(ADMIN_ID, f"🚀 DEBUG: An hada komai na saqo lafiya. Ana kokarin tura sakon karshe zuwa Telegram na User...")
        except: pass
        ...
        try: bot.send_message(ADMIN_ID, f"✅ DEBUG: SUCCESS! An tura saƙon order zuwa ga user lafiya lau.")
        except: pass

    except Exception as tg_err:
        try: bot.send_message(ADMIN_ID, f"❌ DEBUG: Telegram Error wajen gina/tura saƙon karshe: {str(tg_err)}")
        except: pass
        return
