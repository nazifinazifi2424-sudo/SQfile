j## bot.py  (PostgreSQL SAFE – FULL FIX, nothing removed)
...
# ======= GROUPITEM (IDS + GROUP_KEY SUPPORT | DEBUG TRACE MODE) =========
...
def groupitem_deeplink_handler(msg):
    ...
    bot.send_message(ADMIN_ID, "🚀 GROUPITEM TRIGGERED")

    try:
        uid = msg.from_user.id
        raw = msg.text.split("groupitem_", 1)[1].strip()
        bot.send_message(ADMIN_ID, f"👤 USER: {uid}")
        bot.send_message(ADMIN_ID, f"🔗 RAW DATA: {raw}")
    except Exception as e:
        bot.send_message(ADMIN_ID, f"❌ PARSE ERROR: {e}")
        return
    ...
    if all(x.strip().isdigit() for x in raw.replace("_", ",").split(",")):
        ...
        bot.send_message(ADMIN_ID, "🔎 MODE: IDS")

        sep = "_" if "_" in raw else ","
        item_ids = [int(x) for x in raw.split(sep) if x.strip().isdigit()]
        bot.send_message(ADMIN_ID, f"📦 IDS: {item_ids}")

        if not item_ids:
            bot.send_message(ADMIN_ID, "❌ NO VALID IDS")
            cur.close()
            conn.close()
            return
        ...
        bot.send_message(ADMIN_ID, "🔎 MODE: GROUP_KEY")
        bot.send_message(ADMIN_ID, f"🗝 KEY: {raw}")
    ...
    if not items:
        bot.send_message(ADMIN_ID, "❌ NO ITEMS FOUND")
        cur.close()
        conn.close()
        return

    bot.send_message(ADMIN_ID, f"✅ ITEMS FOUND: {len(items)}")

    # FILE CHECK
    items = [i for i in items if i.get("file_id")]

    if not items:
        bot.send_message(ADMIN_ID, "❌ NO FILE_ID ITEMS")
        cur.close()
        conn.close()
        return
    ...
    try:
        ...
        bot.send_message(ADMIN_ID, f"❌ OWNERSHIP ERROR: {e}")
        cur.close()
        conn.close()
        return

    if owned:
        bot.send_message(ADMIN_ID, "⚠️ USER ALREADY OWNS THIS")
        cur.close()
        conn.close()
        return

    bot.send_message(ADMIN_ID, "✅ OWNERSHIP CLEAR")
    ...
    if total <= 0:
        bot.send_message(ADMIN_ID, "❌ INVALID TOTAL")
        cur.close()
        conn.close()
        return
    ...
    try:
        ...
        bot.send_message(ADMIN_ID, f"❌ REUSE CHECK ERROR: {e}")
        cur.close()
        conn.close()
        return
    ...
    if row:
        order_id = row["id"]
        bot.send_message(ADMIN_ID, f"♻️ REUSING ORDER: {order_id}")
    else:
        order_id = str(uuid.uuid4())
        bot.send_message(ADMIN_ID, f"🆕 CREATING ORDER: {order_id}")
        ...
        try:
            ...
            conn.commit()
            bot.send_message(ADMIN_ID, "✅ ORDER COMMITTED")
        except Exception as e:
            conn.rollback()
            bot.send_message(ADMIN_ID, f"❌ ORDER INSERT ERROR: {e}")
            cur.close()
            conn.close()
            return
    ...
    try:
        ...
        bot.send_message(ADMIN_ID, f"❌ FLUTTERWAVE ERROR: {e}")
        cur.close()
        conn.close()
        return

    if not pay_url:
        bot.send_message(ADMIN_ID, "❌ FLUTTERWAVE RETURNED NONE")
        cur.close()
        conn.close()
        return

    bot.send_message(ADMIN_ID, "✅ PAYMENT LINK CREATED")
    ...
    bot.send_message(ADMIN_ID, "🎉 GROUPITEM FINISHED SUCCESSFULLY")
